...
- **input の `type` はゲートとして働く。** **Text（テキスト）種別**のカスタムフィールドを使用した場合、`input[type=text]` にのみ充填され、`search` / `email` / `password` / `textarea` には充填されない。型は一致のシグナルではなく、カスタムフィールドの種別と対応しているかを判定するゲートと考えられる（他の種別は未検証）
...
| 問い | 状態 |
| --- | --- |
| 他の種別のカスタムフィールドは、対応する `type` に充填されるか | 未検証。1Password のカスタムフィールドには テキスト / URL / メール / 住所 / 日付 / ワンタイムパスワード / パスワード / 電話 / サインイン の種別がある。種別違いの `OP_TEST_TOKEN` を作り `?only=5-D` 等を実行することで、コード変更なしに検証できる |
