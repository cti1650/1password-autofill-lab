...
> [!NOTE]
> **カスタムフィールドの種類は結果に影響します。** 1Password には テキスト / URL / メール / 住所 / 日付 / ワンタイムパスワード / パスワード / 電話 / サインイン の種別があり、種別ごとに充填先の input `type` が変わると考えられます。
> 本ラボの記録済み結果はすべて **Text 種別**で取得したものです。他の種別で試す場合は、[結果の記録](#結果の記録)に**使用した種別を必ず併記**してください。
...
- **input の `type` はゲートとして働く。** 本ラボは 1Password 側で **Text（テキスト）種別**のカスタムフィールドのみを使用しており、その条件下では `input[type=text]` にしか充填されなかった（`search` / `email` / `password` / `textarea` は不可）。型は一致のシグナルではなく、**カスタムフィールドの種別と対応しているかを判定するゲート**と考えられる
- **一致に使われる情報源**: `id` / `name` / `placeholder` / `title` / `aria-label` / `aria-labelledby` / `label`（正式な関連付けの有無を問わない）/ **input より前方の可視テキスト**
- **不可視テキストは無視される**（`display:none`、`hidden` 属性）
...
| 問い | 状態 |
| --- | --- |
| **他の種別のカスタムフィールドは、対応する `type` に充填されるか** | **未検証。最優先。** 1Password のカスタムフィールドには テキスト / URL / メール / 住所 / 日付 / ワンタイムパスワード / パスワード / 電話 / サインイン の種別があり、**メール種別なら `input[type=email]`、パスワード種別なら `input[type=password]` に充填される可能性が高い**。本ラボの「`text` 以外は不可」という結果は Text 種別に限った話であり、一般化できない。コード変更なしで検証可能（種別違いの `OP_TEST_TOKEN` を作り `?only=5-D` 等を実行するだけ） |
...
└── docs/
    ├── findings.md     # 観測結果の詳細、仮説、次に必要な実験
